...
CREATE TABLE users (
  ...
  default_mess_id INT,          -- FK added after messes table
...
CREATE TABLE locations (
  ...
  category    VARCHAR(20)  NOT NULL
              CHECK (category IN ('academic','food','sports','hostel','service','medical','library')),
...
CREATE INDEX idx_checkins_location_time ON checkins(location_id, checked_in_at);
CREATE INDEX idx_checkins_user_date ON checkins(user_id, checked_in_at);
...
CREATE UNIQUE INDEX idx_mess_menu_unique ON mess_menu(mess_id, day_of_week, meal_type);
...
-- Add FK from users to messes (after messes table exists)
...
CREATE TABLE club_members (
  ...
  role      VARCHAR(20) DEFAULT 'member'
            CHECK (role IN ('president','moderator','member')),
...
CREATE TABLE posts (
  ...
  ref_id       INT,      -- event_id / location_id / lost_found_id / market_id depending on post_type
  ref_type     VARCHAR(30), -- 'event' / 'location' / 'lost_found' / 'market'
  created_at   TIMESTAMPTZ DEFAULT NOW(),
  updated_at   TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE post_reactions (
  ...
  reaction_type VARCHAR(10) NOT NULL
                CHECK (reaction_type IN ('fire','heart','laugh','clap','wow')),
  created_at    TIMESTAMPTZ DEFAULT NOW(),
...
CREATE TABLE notifications (
  ...
  type       VARCHAR(30) NOT NULL
             CHECK (type IN ('like','reaction','comment','reply','follow','event_rsvp','club_join',
                             'announcement','mess_booked','new_post','mention','memory_added')),
  content    TEXT NOT NULL,
  ref_id     INT,
  ref_type   VARCHAR(30),
...
CREATE TABLE announcements (
  ...
  department  VARCHAR(60),  -- NULL = all departments
  created_by  INT REFERENCES users(id),
